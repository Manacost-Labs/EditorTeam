...
def test_console_script_installed():
    r = subprocess.run([sys.executable, "-m", "editorteam.cli", "profiles"],
...
def test_flag_works_before_and_after_subcommand(capsys, bg_file):
    ...
    for argv in (["--format", "json", "audit", str(bg_file)],
...
def test_fail_on_changes_exit_code(capsys, bg_file):
    code_default = main(["audit", str(bg_file), "--profile", "constructed-guide",
                         "--format", "json"])
    capsys.readouterr()
    code_strict = main(["audit", str(bg_file), "--profile", "constructed-guide",
                        "--format", "json", "--fail-on", "likely"])
    capsys.readouterr()
